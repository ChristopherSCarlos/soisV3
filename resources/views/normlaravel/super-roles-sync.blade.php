...
<div class="flex flex-col p-5">
	<div class="max-w-lg rounded overflow-hidden shadow-lg">
		...
		<form name="add-users" id="add-users" method="post" action="{{ route('roles/syncPage', $roles->role_id ) }}">
			@csrf
			@method('PUT')
            <div class="mt-4" style="overflow: auto; height:80vh;">
                <label value="">{{ __('PErmission List') }}</label>

                @foreach($perms as $permissions)
                    <input type="checkbox" id="{{$permissions->permission_id}}" name="category[]" value="{{$permissions->permission_id}}">
